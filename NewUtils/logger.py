...
class SoloPlot:
    def __init__(self,MegaDict):
        ...
        self.calculate_x_path()

    def reset_colors(self):
        self.color_cycle=cycle( plt.rcParams['axes.prop_cycle'].by_key()['color'])

    def calculate_x_path(self):
        """
        Calcula a posición X do vehículo en función da lonxitude e latitud
        """
        x = np.array(self.GPS['Lon'])
        y = np.array(self.GPS['Lat'])
        ...
        unit_vector = np.linalg.norm(np.array([1, m]))
    ...
    def plot_GPS(self,mode:Literal['NSats','Lat','Lon','Alt','Speed','VZ','Yaw','Ground_Position','3D_Position','Path_Position']):
        self.reset_colors()
        match mode:
