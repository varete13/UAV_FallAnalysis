...
def Pandas_speed(fichero)->pd.DataFrame:
    columnas=['Fecha','Hora']
    ...
    df_0=df_0[df_0['medidas']=='mavlink_vfr_hud_t'].dropna(axis='columns')

    Fecha=df_0.iloc[0,0]
    df_0=df_0.iloc[:,1:]
    df_0=df_0.drop([chr(i) for i in range(ord('A'),ord('A')+8)]+[13,14,15,16,17],axis=1)
    Horas=df_0.iloc[:,0]
    df_0=df_0.iloc[:,2:]
