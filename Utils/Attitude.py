...
def Pandas_attitude(fichero:str)->pd.DataFrame:
    """
    Lectura del punto .txt extraido de ardupilot para la lectura de

    param: fichero Direccion del .txt a examinar
    ...
    df_0=df_0[df_0['medidas']=='mavlink_attitude_t'].dropna(axis='columns')
    Fecha=df_0.iloc[0,0]
    df_0=df_0.iloc[:,1:]
    df_0=df_0.drop([chr(i) for i in range(ord('A'),ord('A')+8)]+[15,16,17,18,19],axis=1)
    Horas=df_0.iloc[:,0]
    df_0=df_0.iloc[:,2:]
